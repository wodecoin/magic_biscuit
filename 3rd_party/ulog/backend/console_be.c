...
static void ulog_test(void)
{
    ulog_d(TAG, "Debug message, value=%d", 123);
    ulog_i(TAG, "Info message");
    ulog_w(TAG, "Warning message");
    ulog_e(TAG, "Error message");
}
INIT_APP_EXPORT(ulog_console_backend_init);
MSH_CMD_EXPORT(ulog_test, demo log output);
